Component({
  ...
  data: {
    ...
    _img_top: dd.getSystemInfoSync().windowHeight / 2, //图片上边距
    _img_left: dd.getSystemInfoSync().windowWidth / 2, //图片左边距
    watch: {
      //监听截取框宽高变化
      width(value, that) {
  ...
  attached() {
    this.data.info = dd.getSystemInfoSync();
    //启用数据监听
    this._watcher();
  ...
  methods: {
    ...
    imgReset() {
      this.setData({
        scale: 1,
        angle: 0,
        _img_top: dd.getSystemInfoSync().windowHeight / 2,
        _img_left: dd.getSystemInfoSync().windowWidth / 2,
      })
    },
    /**
     * 加载（更换）图片
    ...
    _initCanvas() {
      //初始化canvas
      if (!this.data.ctx){
        this.data.ctx = dd.createCanvasContext("image-cropper", this);
      }
    },
    /**
     * 根据开发者设置的图片目标尺寸计算实际尺寸
